...
As samples get smaller: $N \downarrow \dots$
...
### Comparatively... in LARGE smaples
...
- Larger area in **tails** (%) for any value of $t$ corresponding to $z$
  - $CV_t \gt CV_z$, for a given $\alpha$
...
.center[Estimated $SE_M$ is the amount a sample's observed **mean** <br>may have deviated from <br> the true or population value <br>just due to random chance variation due to sampling.]
...
CI expresses our .nicegreen[**confidence**] in a statistic & the .nicegreen[*width*] depends on $SE_M$ and $CV_t$
