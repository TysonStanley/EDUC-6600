...
fit_audience <- audience_long %>% 
  afex::aov_4(blood_pressure ~ 1 + (audience|id),
              data = .,
              anova_table = list(correction = "none",
                                 es = c("ges", "pes")))
...
```{r}
# RM ANOVA: display all Sums-of-Squares components
fit_textbook <- textbook_long %>% 
  afex::aov_4(quiz ~ 1 + (book|block),
              data = .) 
...
fit_book1way <- textbook_long %>% 
  afex::aov_4(quiz ~ book + (1|id),
              data = .) 
...
**DIRECTIONS:** Perform a Repeated Measures ANOVA for anxiety at all three time points to determine if the experiment had an effect. Make sure to preceed teh ANOVA with a `dplyr::filter()` step to restrict to just `genderF == "Female`.  Save it as the name `fit_anx_female` and then use the `summary()` function display additional output.
...
ihno_anx_long %>% 
  dplyr::filter(genderF == "Female" & time %in% c("baseline", "pre-quiz")) %>% 
  t.test(anxiety ~ time,
         data = .,
         paired = TRUE)
```
...
ihno_anx_long %>% 
  dplyr::filter(genderF == "Female" & time %in% c("baseline", "post-quiz")) %>% 
  t.test(anxiety ~ time,
         data = .,
         paired = TRUE)
```
...
ihno_anx_long %>% 
  dplyr::filter(genderF == "Female" & time %in% c("pre-quiz", "post-quiz")) %>% 
  t.test(anxiety ~ time,
         data = .,
         paired = TRUE) 
```
...
ihno_statquiz_long %>% 
  afex::aov_4(s_quiz ~ 1 + (time|sub_num),
              data = .)
```
...
fit_tasks <- tasks_long %>% 
  afex::aov_4(completed ~ type_task + (noise|id),
              data = .,
              anova_table = list(correction = "none",
                                 es = c("ges", "pes"))) 
...
```{r}
# Mixed ANOVA: display all Sums-of-Squares components
fit_ano <- anograms_long %>% 
  afex::aov_4(correct ~ feedback + (difficulty|id),
            data = .) 
...
a### 16c-1a Mixed Design ANOVA: with main effect post hocs
...
```{r}
# Mixe ANOVA:  with sphericity tests and corrections
fit_hr_major <- ihno_hr_long %>%   
  afex::aov_4(hr ~ genderF + (time|sub_num),
            data = .,
            anova_table = list(es = c("ges", "pes")))

summary(fit_hr_major)
```
...
fit_statquiz <- ihno_statquiz_long %>%   
  afex::aov_4(s_quiz ~ exp_condF + (time|sub_num),
            data = .,
            anova_table = list(es = c("ges", "pes")))

summary(fit_statquiz)
```
